<template>
  <div class="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-blue-50 to-green-100 font-poppins">
    <div class="bg-white p-8 rounded-xl shadow-lg w-full max-w-sm transition-all duration-300 hover:shadow-2xl">
  <h2 class="text-3xl font-extrabold mb-6 text-gray-800 text-center tracking-tight">Connexion</h2>
  ...
  <button type="submit" class="w-full bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 rounded transition-colors duration-200" :disabled="auth.loading">Se connecter</button>
      </form>
  <div v-if="auth.error" class="mt-4 text-red-500 text-center animate-pulse">{{ auth.error }}</div>
      <div class="mt-6 text-center">
        ...
        <button @click="router.push('/register')" class="ml-2 text-blue-600 hover:underline font-semibold transition-colors duration-200">Créer un compte</button>
      </div>
    </div>
  </div>
</template>
